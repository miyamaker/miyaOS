...
vec4 getSunColor( in vec3 dir, inout float inside ) {
    ...
    return vec4(mix(vec3(0.998, 0.108, 0.47), vec3(0.988, 0.769, 0.176), sunHeight), 0);
}
...
vec3 getSpaceColor( in vec3 dir ) {
    ...
    return mix(color, lightColor, 0.) + vec3(1)*frac2;
}
...
vec3 getWaveColor( in vec3 p, in vec3 projClosest, in vec3 projSecondClosest,
    ...
    // get the reflection
    vec3 norm = normal(vec3(0.0,0.0,0.0), projClosest);
...
void main() {
	...
	dir.xy *= rot(PI*.5);
	dir.xz *= rot(3.16-(-4.15*1.5)+sin(iTime*0.785)*0.0);
    dir.yz *= rot(sin(iTime*0.287)*0.0);
        ...
        for (int steps = 0 ; steps < STEPS ; steps++) {
            ...
            if ( dist < 0.01 || steps == STEPS-1 ) {
                color = getWaveColor(p, projClosest, projSecondClosest,
                                     dir, totdist, vec2(gl_FragCoord.x, gl_FragCoord.y));
                break;
            }
        }
	}
