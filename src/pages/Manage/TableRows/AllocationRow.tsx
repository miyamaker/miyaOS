...
export default function AllocationRow({
  ...
  const { allocation, increaseAllocation, refetch } = useAllocation({ address: collection })
  ...
  const openInput = () => {
    setInput(formatUnits(allocation, 2))
    onActive()
  }
  ...
  const [isValidated, validated, errors] = useMemo(() => {
    const validateWithMax = z.bigint().gt(0n).gt(allocation).lt(10000n)
  ...
  const handleUpdate = async () => {
    ...
    setTransacting(true)
    try {
      const result = await increaseAllocation(validated)
  ...
  return (
    <tr>
      ...
      {active ? (
        ...
        <td onClick={() => openInput()}>{formatUnits(allocation, 2)}%</td>
      )}
      <td style={{ padding: 0 }}>
        <ConnectedButton
          style={{
